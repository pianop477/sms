...
class HomeController extends Controller
{
    ...
    public function index()
    {
        ...
        elseif ($user->usertype == 3) {
            ...
            // Kuweka data kwa ajili ya chart
            $attendanceByClassData = $classesWithAttendance;
            ...
            // REINITIALIZE - Anzisha upya kuwa na EMPTY array
            $classTeacherAttendance = []; // <-- BADILISHA JINA LA VARIABLE
            ...
            if ($classTeacher) {
                ...
                $attendanceStats = Attendance::query()
                    ...
                    ->where('attendances.teacher_id', $teacher->id) // Mwalimu aliyelogin
                    ->whereDate('attendances.attendance_date', $today)
                    ->get();

                // Initialize array
                $classTeacherAttendance = [
                    'male' => ['present' => 0, 'absent' => 0, 'permission' => 0],
                    'female' => ['present' => 0, 'absent' => 0, 'permission' => 0],
                    'has_data' => false // FLAG YA KUANGALIA
                ];

                if ($attendanceStats->count() > 0) {
                    $classTeacherAttendance['has_data'] = true;
                    ...
                    foreach ($attendanceStats as $attendance) {
                        ...
                        if ($gender == 'male') {
                            $classTeacherAttendance['male'][$status]++;
                        } elseif ($gender == 'female') {
                            $classTeacherAttendance['female'][$status]++;
                        }
                    }
                }
            }

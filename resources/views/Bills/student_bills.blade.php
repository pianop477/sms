        ...
        <div class="row mb-4">
            <div class="col-md-8">
                <h4 class="text-primary fw-bold border-bottom pb-2">
                    ...
                    Payment History for <span
                        style="text-decoration:underline"><strong>{{ ucwords(strtolower($students->first_name . ' ' . $students->last_name)) }}</strong></span>
                </h4>
            </div>
            <div class="col-md-4 text-md-end align-self-center">
                <a href="{{ route('students.profile', ['student' => Hashids::encode($students->id)]) }}"
                    ...
                    <div class="card-body">
                        ...
                        <div class="mt-4">
                            ...
                            <div class="d-flex justify-content-between align-items-center mb-2">
                                ...
                                <span
                                    class="badge bg-primary text-white">{{ $paymentRecords->where('type', 'invoice')->count() }}</span>
                            </div>
                            <div class="d-flex justify-content-between align-items-center mb-2">
                                <span class="text-muted small">Payments:</span>
                                <span
                                    class="badge bg-success text-white">{{ $paymentRecords->where('type', 'payment')->count() }}</span>
                            </div>
                        </div>
                    </div>
                </div>
                                ...
                                <tbody>
                                    ...
                                    @forelse($paymentRecords as $record)
                                        @if ($currentControlNumber !== $record['control_number'])
                                            @php
                                                $currentControlNumber = $record['control_number'];
                                            @endphp
                                            <!-- Invoice Row -->
                                            <tr class="invoice-row">
                                                ...
                                                <td>
                                                    @if ($record['status'] == 'active')
                                                        <span
                                                            class="badge bg-primary text-white">{{ strtoupper($record['status']) }}</span>
                                                    @elseif ($record['status'] == 'cancelled')
                                                        <span
                                                            class="badge bg-warning text-primary">{{ strtoupper($record['status']) }}</span>
                                                    @elseif ($record['status'] == 'expired')
                                                        <span
                                                            class="badge bg-danger text-white">{{ strtoupper($record['status']) }}</span>
                                                    @else
                                                        <span
                                                            class="badge bg-success text-white">{{ strtoupper($record['status']) }}</span>
                                                    @endif
                                                </td>
                                        ...
                                        @if ($record['type'] == 'payment')
                                            <tr class="payment-row">
                                                ...
                                                <td class="text-muted small text-capitalize">{{ $record['service_name'] }}
                                                </td>
                                                <td class="text-muted amount-cell">0</td>
                                                <td class="amount-cell text-success">{{ number_format($record['amount']) }}
                                                </td>
                                                <td>
                                                    <span
                                                        class="badge bg-info text-white">{{ strtoupper($record['status']) }}</span>
                                                </td>
                                                <td>-</td>
                                            </tr>
                                        @endif

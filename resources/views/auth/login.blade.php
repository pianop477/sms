...
<body>
    ...
    <div class="login-card">
        @if (session('error'))
            <div
                style="background: rgba(239, 68, 68, 0.1); padding: 10px; border-radius: 6px; border-left: 3px solid #ef4444; margin-bottom: 16px; font-size: 13px;">
        ...
        <form id="loginForm" method="POST" action="{{ route('login') }}">
            ...
            <div class="form-group">
                ...
                <input type="text" id="login" name="username" class="form-control"
                    placeholder="user@example.com or 075XXXXXXX" required />
            </div>
    ...
    <div class="footer">
        @php
            ...
            $currentYear = date('Y');
        @endphp
        &copy; {{ $startYear == $currentYear ? $startYear : $startYear . ' - ' . $currentYear }} ShuleApp. All rights
        reserved.
    </div>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            function updateBiometricUI() {
                ...
                if (bioBtnText) bioBtnText.textContent = bioSettings.registered ? 'Use Biometric' :
                    'Set Up Biometrics';
                ...
                setupBioBtn.addEventListener('click', async function(e) {
                    ...
                    `;
                        ...
                        document.getElementById('verifyOtpBtn').addEventListener('click', async () => {
                            ...
                            const verifyBtnText = document.getElementById(
                                'verifyOtpBtnText');
                            ...
                            try {
                                ...
                                try {
                                    ...
                                    // Fallback for PWA
                                    verifyResponse = await fetch(window.location.origin +
                                ...
                                if (verifyData.requires_browser) {
                                    showToast('Please complete setup in your browser',
                                        'info');
                                    window.open(window.location.origin +
                                        '/biometric/setup?token=' + verifyData.token,
                                        '_blank');
                                } else {
                                ...
                                showToast(error.message ||
                                    'Verification failed. Please try in browser if using PWA.',
                                    'error');
                            } finally {
                ...
                try {
                    ...
                    const verificationRes = await fetch("/webauthn/register/verify", {
                        ...
                        body: JSON.stringify({
                            ...
                            response: {
                                attestationObject: uint8ArrayToBase64url(credential.response
                                    .attestationObject),
                                clientDataJSON: uint8ArrayToBase64url(credential.response
                                    .clientDataJSON)
                            },
                            username: username
                        })
            ...
            if (bioBtn) {
                bioBtn.addEventListener('click', async function() {
                    ...
                    if (!username) {
                        showToast('Click Setup Biometric button below to register your device',
                            'warning');
                        return;
                    }
                            ...
                            verifyRes = await fetch("/webauthn/login/verify", {
                                ...
                                body: JSON.stringify({
                                    ...
                                    response: {
                                        ...
                                        clientDataJSON: uint8ArrayToBase64url(assertion
                                            .response.clientDataJSON),
                                        signature: uint8ArrayToBase64url(assertion
                                            .response.signature),
                                        userHandle: assertion.response.userHandle ?
                                            uint8ArrayToBase64url(assertion.response
                                                .userHandle) : null
                                    }
                                })
                            });

    ...
    <table class="header">
        <tr>
            ...
            <td width="70%" class="school-info">
                ...
                <h5 style="margin:0; padding:0;">{{ $schoolInfo->postal_address }}, {{ $schoolInfo->postal_name }} -
                    {{ $schoolInfo->country }}</h5>
                ...
                @php
                    ...
                    if (!empty($imageName) && file_exists($imagePath)) {
                        ...
                        $avatarImage = storage_path(
                            'app/public/students/' . ($students->gender == 'male' ? 'student.jpg' : 'student.jpg'),
                        );
                    }
                @endphp
                <img src="{{ $avatarImage }}" alt="" width="80" class="profile-img">
            </td>
        </tr>
    </table>
    <table class="report-header">
        <tr>
            <td>
                ...
                <h5 style="margin:0; padding:0;"> {{ $reports->title }} Assessment -
                    {{ \Carbon\Carbon::parse($reports->created_at)->format('d/m/Y') }}</h5>
            </td>
        </tr>
    </table>
    <p style="padding: 3px; background:rgb(187, 163, 56); text-align:center; font-size: 14px;"><strong>Student's
            Information</strong></p>
    <table class="student-info">
        <tr>
            <td width="50%">
                <strong>Admission Number:</strong> <span class="">{{ $students->admission_number }}</span><br>
                <strong>Student Name:</strong> <span class="">{{ $students->first_name }}
                    {{ $students->middle_name }} {{ $students->last_name }}</span><br>
    ...
    @if ($reports->combine_option === 'individual')
        <table class="report-table">
            ...
            <tbody>
                @foreach ($finalData as $subject)
                    <tr>
                        <td class="subject-name" style="text-transform: capitalize">
                            {{ ucwords(strtolower($subject['subjectName'])) }} <span class=""
                                style="text-transform: uppercase">({{ ucwords(strtoupper($subject['subjectCode'])) }})</span>
                        </td>
                        <td class="teacher-name">{{ ucwords(strtolower($subject['teacher'])) }}</td>
                        @foreach ($examHeaders as $exam)
                            <td class="exam-score text-center">
                                {{ $subject['examScores'][$exam['abbr'] . '_' . $exam['date']] ?? 'X' }}</td>
                        @endforeach
                        <td>{{ $subject['total'] }}</td>
                ...
                <tr class="summary-row">
                    <td colspan="{{ count($examHeaders) + 7 }}" style="background: rgb(187, 163, 56)">Overall
                ...
                <tr class="summary-row">
                    ...
                    <td colspan="3">
                        Position: <strong style="text-decoration:underline">{{ $generalPosition }}</strong> out of
                        <strong style="text-decoration:underline">{{ $totalStudents }}</strong><br>
                    </td>
        ...
        @if (count($examSpecifications) > 0)
            <table class="report-table" style="margin-top: 20px;">
                <tbody>
                    <tr>
                        <td colspan="7" style="background: rgb(187, 163, 56); font-size: 12px"><strong>Examinations
                                codes Key</strong></td>
                    </tr>
                    <tr>
        ...
        <table class="report-table" style="font-size: 13px;">
            ...
            <tbody>
                ...
                <tr class="summary-row">
                    ...
                    <td colspan="2">
                        Position: <strong style="text-decoration:underline">{{ $generalPosition }}</strong> out of
                        <strong style="text-decoration:underline">{{ $totalStudents }}</strong>
                    </td>
                    <td colspan="2" class="text-center">
                        General Remarks:
                        @if ($results->first()->marking_style === 1)

                        ...
                        <table class="table table-heslb table-bordered" id="activeTable" width="100%">
                            ...
                            <tbody>
                                ...
                                <tr>
                                    ...
                                    <td>{{ $deduction['end_date'] ?
                                        \Carbon\Carbon::parse($deduction['end_date'])->format('d/m/Y') : 'Ongoing' }}
                                    </td>
                        ...
                        <table class="table table-heslb table-bordered" id="inactiveTable" width="100%">
                            ...
                            <tbody>
                                ...
                                <tr>
                                    ...
                                    <td>{{ $deduction['end_date'] ?
                                        \Carbon\Carbon::parse($deduction['end_date'])->format('d/m/Y') : 'N/A' }}
                                    </td>
...
<script>
    ...
    let isSubmitting = false; // NEW: To prevent double submission
            ...
            success: function(response) {
                if (response.success) {
                    ...
                    if (data.inactive && data.inactive.length > 0) {
                        ...
                        $('#inactiveTable').parent().parent().show();
                    } else {
                        ...
                        `);
                        $('#inactiveTable').parent().parent().hide();
                    }
    ...
    // ==================== HESLB FUNCTIONS (ENHANCED WITH SUBMISSION PROTECTION) ====================

    // Function to get the button element that triggered the action
    ...
    function stopDeduction(id) {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                // Find the button that triggered this action (will be disabled later)
                const buttons = document.querySelectorAll(`button[onclick*="stopDeduction(${id})"]`);
                const btn = buttons.length > 0 ? buttons[0] : null;
                submitForm('/heslb/' + id + '/stop', {}, btn);
            }
        });
    }
    ...
    function updateAmount(id, currentAmount) {
        Swal.fire({
            ...
            if (result.isConfirmed && result.value) {
                const buttons = document.querySelectorAll(`button[onclick*="updateAmount(${id},"]`);
                const btn = buttons.length > 0 ? buttons[0] : null;
    ...
    // ENHANCED submitForm with double-submission protection
    function submitForm(url, data = {}, button = null) {
        // If already submitting, block
        if (isSubmitting) {
            return;
        }

        // If button is provided, disable it and show spinner
        let originalHtml = '';
        if (button) {
            isSubmitting = true;
            ...
            // Try to find a submit button in the form (fallback)
            ...
            if (form) {
                ...
                if (submitBtn) {
                    ...
                    // We'll re-enable via timeout or manual reset later
                }
            }
        }

        // Set timeout to re-enable button if submission fails (network timeout)
        ...
        // Submit and re-enable button if there's an error (we'll catch via page reload or timeout)
        // The timeout will handle re-enable if no redirect occurs.
        form.submit();

        // Clean up after 15 seconds (if the form hasn't redirected)
        // The timeout above handles that.
        // Also, add an event listener to reset if the page doesn't unload (rare)

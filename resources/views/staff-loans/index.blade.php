            ...
            <div class="card">
                ...
                <div class="card-body">
                    ...
                    <h6 class="mb-3">
                        ...
                        <span class="badge bg-secondary ms-2" id="pendingCount">{{ count($deductions['pending'] ?? [])
                            }}</span>
                    </h6>
                    ...
                    <div id="historySection" @if (count($deductions['history'] ?? [])==0) style="display: none;" @endif>
                        <h6 class="mb-3 mt-4">
                            ...
                            <span class="badge bg-secondary ms-2" id="historyCount">{{ count($deductions['history'] ??
                                []) }}</span>
                        </h6>
                        <div class="table-responsive">
                    ...
                    <div class="row">
                        ...
                        <div class="col-md-6 mb-3">
                            ...
                            <select name="staff_type" class="form-select @error('staff_type') is-invalid @enderror"
                                ...
                                <option value="Transport Staff" {{ old('staff_type')=='Transport Staff' ? 'selected'
                                    : '' }}>Transport Staff
                                </option>
                        ...
                        <div class="col-md-6 mb-3" id="recurringMonthsDiv" style="display: none;">
                            ...
                            <select name="recurring_months" id="recurringMonthsSelect"
                                ...
                                @for ($i = 1; $i <= 36; $i++) <option value="{{ $i }}" {{ old('recurring_months')==$i
                                    ? 'selected' : '' }}>
                                    {{ $i }} month{{ $i > 1 ? 's' : '' }}
                                    </option>
                                    @endfor
                        ...
                        <div class="col-12 mb-3">
                            ...
                            <textarea name="authorization_notes"
                                class="form-control @error('authorization_notes') is-invalid @enderror" rows="2"
                    ...
                    <div class="row">
                        ...
                        <div class="col-md-6 mb-3" id="edit_recurringMonthsDiv" style="display: none;">
                            ...
                            <select name="recurring_months" id="edit_recurringMonthsSelect" class="form-select">
                                <option value="">Select months</option>
                                @for ($i = 1; $i <= 36; $i++) <option value="{{ $i }}">{{ $i }}
                                    month{{ $i > 1 ? 's' : '' }}</option>
                                    @endfor
                            </select>
                        ...
                        <div class="col-12 mb-3">
                            <label class="form-label">Authorization Notes</label>
                            <textarea name="authorization_notes" id="edit_authorization_notes" class="form-control"
                                rows="2"></textarea>
                        </div>
                    </div>
                </div>
...
{{-- 6. SIXTH: Your custom script --}}
<script>
    // ==================== GLOBAL FLAGS FOR DUPLICATE SUBMISSION ====================
    let isAddSubmitting = false;
    let isEditSubmitting = false;

    // ==================== GLOBAL FUNCTIONS (AS ORIGINAL) ====================
            ...
            .then(data => {
                ...
                if (data.success && data.data) {
                    ...
                    if (isRecurring && deduction.recurring_months) {
                        ...
                        const total = (parseFloat(deduction.amount) || 0) * (parseInt(deduction.recurring_months) || 0);
    ...
    // ==================== DATATABLES INITIALIZATION (AS ORIGINAL) ====================
    ...
    $(document).ready(function() {
        ...
        // ==================== ADD FORM SUBMISSION (NEW WITH DUPLICATE PROTECTION) ====================
        const addForm = document.querySelector('#addLoanModal form');
        const addSubmitBtn = addForm?.querySelector('button[type="submit"]');

        if (addForm && addSubmitBtn) {
            // Re-enable button if validation fails (bootstrap native validation)
            addForm.querySelectorAll('input, select, textarea').forEach(input => {
                input.addEventListener('invalid', function() {
                    if (isAddSubmitting) {
                        isAddSubmitting = false;
                        addSubmitBtn.disabled = false;
                        addSubmitBtn.innerHTML = '<i class="fas fa-save me-1"></i> Add Deduction';
                    }
                });
            });
            ...
            addForm.addEventListener('submit', function(e) {
                ...
                // Let browser validation run – if invalid, do nothing
                ...
                addSubmitBtn.innerHTML = '<i class="fas fa-spinner fa-spin me-1"></i> Saving...';

                // Allow form to submit normally (we will re-enable on timeout if needed)
                // We don't prevent default here – the form will submit naturally.
                // However, we set a timeout to re-enable if submission does not complete.
                setTimeout(function() {
                    if (isAddSubmitting) {
                        isAddSubmitting = false;
                        addSubmitBtn.disabled = false;
        ...
        // ==================== EDIT FORM SUBMISSION (ENHANCED WITH DUPLICATE PROTECTION) ====================
        $('#editDeductionForm').on('submit', function(e) {
            ...
            // Timeout to re-enable if server doesn't respond
            ...
            $.ajax({
                ...
                success: function(response) {
                    ...
                    if (isSuccess) {
                        const editModal = bootstrap.Modal.getInstance(document.getElementById('editLoanModal'));
                        if (editModal) editModal.hide();

                        Swal.fire({
                            icon: 'success',
                            title: 'Updated!',
                            text: response.message || 'Deduction updated successfully',
                            timer: 1500,
                            showConfirmButton: false
                        }).then(() => {
        ...
        // ==================== EDIT MODAL RECURRING LOGIC (AS ORIGINAL) ====================
        ...
        $(document).on('change', '#edit_deductionType', function() {
            ...
            if (type === 'loan') {
                ...
                $('#edit_amountHelpText').html('<strong>Monthly installment amount</strong> - This amount will be deducted each month');
            } else {
                $('#edit_recurringOptionDiv').hide();
                $('#edit_recurringMonthsDiv').hide();
                $('#edit_isRecurring').prop('checked', false);
                $('#edit_amountHelpText').html('<strong>Total amount to deduct</strong> - One-time deduction');
            }
        });
    ...
    // ==================== ADD FORM RECURRING LOGIC (AS ORIGINAL) ====================
    ...
    // ==================== SWEETALERT NOTIFICATIONS (AS ORIGINAL) ====================
    @if (session('success'))
        Swal.fire({
            icon: 'success',
            title: 'Success!',

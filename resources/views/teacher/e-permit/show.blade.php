...
<style>
    ...
    .status-pending-class-teacher { background: #f59e0b; color: #fff; }
    .status-pending-duty-teacher { background: #f59e0b; color: #fff; }
    .status-pending-academic { background: #f59e0b; color: #fff; }
    .status-pending-head { background: #f59e0b; color: #fff; }
    ...
    .timeline-icon.blue { background: #3b82f6; color: white; border-color: #3b82f6; }
    .timeline-icon.green { background: #22c55e; color: white; border-color: #22c55e; }
    .timeline-icon.red { background: #ef4444; color: white; border-color: #ef4444; }
    .timeline-icon.orange { background: #f59e0b; color: white; border-color: #f59e0b; }
    .timeline-icon.purple { background: #8b5cf6; color: white; border-color: #8b5cf6; }
    ...
    .info-badge-class-teacher { background: #dbeafe; color: #1e40af; }
    ...
    @media (max-width: 768px) {
        ...
        .timeline-icon { width: 30px; height: 30px; left: -25px; }
        .timeline-icon i { font-size: 12px; }
        .btn-approve, .btn-reject, .btn-back { padding: 8px 16px; font-size: 0.85rem; }
    }
</style>
        ...
        <div class="col-12">
            ...
            <div class="card mb-4">
                ...
                <div class="card-body">
                        ...
                        <div>
                            @php
                                ...
                                $statusClass = match($permit->status) {
                                    'pending_class_teacher', 'pending_duty_teacher', 'pending_academic', 'pending_head' => 'pending',
                                    'approved' => 'approved',
                                    'rejected' => 'rejected',
                                    'completed' => 'completed',
                                    default => 'pending'
                            ...
                            <span class="permit-status status-{{ $statusClass }}">
                                <i class="fas {{ $permit->status === 'approved' ? 'fa-check-circle' : ($permit->status === 'rejected' ? 'fa-times-circle' : 'fa-clock') }} me-1"></i>
                                {{ $statusDisplay }}
                            </span>
                        </div>
                    </div>
                                        ...
                                        <div class="info-grid">
                                            <div class="info-item">
                                                ...
                                                <div>
                                                    ...
                                                    <div class="info-value">{{ ucfirst($permit->student->first_name) }} {{ ucfirst($permit->student->middle_name) }} {{ ucfirst($permit->student->last_name) }}</div>
                                                </div>
                                            </div>
                                            <div class="info-item">
                                            ...
                                            <div class="info-item">
                                                ...
                                                <div>
                                                    ...
                                                    <div class="info-value">{{ strtoupper($permit->student->class->class_name ?? 'N/A') }}</div>
                                                </div>
                                            </div>
                                            <div class="info-item">
                                            ...
                                            <div class="info-item">
                                                ...
                                                <div>
                                                    ...
                                                    <div class="info-value">{{ ucfirst($permit->student->gender ?? 'N/A') }}</div>
                                                </div>
                                            </div>
                                        </div>
                                    </div>
                            ...
                            <div class="info-card">
                                ...
                                <div class="info-grid">
                                    <div class="info-item">
                                        ...
                                        <div>
                                            <div class="info-label">Jina Kamili</div>
                                            <div class="info-value">{{ ucwords(strtolower($permit->guardian_name)) }}</div>
                                        </div>
                                    </div>
                                    <div class="info-item">
                                ...
                                <div class="info-grid">
                                    ...
                                    <div class="info-item">
                                        ...
                                        <div>
                                            ...
                                            <div class="info-value">{{ \Carbon\Carbon::parse($permit->departure_date)->format('d/m/Y') }}</div>
                                        </div>
                                    </div>
                                    <div class="info-item">
                                        ...
                                        <div>
                                            ...
                                            <div class="info-value">{{ \Carbon\Carbon::parse($permit->departure_time)->format('H:i') }}</div>
                                        </div>
                                    </div>
                                    <div class="info-item">
                                        ...
                                        <div>
                                            ...
                                            <div class="info-value">{{ \Carbon\Carbon::parse($permit->expected_return_date)->format('d/m/Y') }}</div>
                                        </div>
                                    </div>
                                </div>
                            </div>
                                        ...
                                        <div class="timeline-item">
                                            ...
                                            <div class="timeline-content">
                                                ...
                                                <div class="timeline-meta">
                                                    ...
                                                    <i class="fas fa-clock"></i> {{ \Carbon\Carbon::parse($item['time'])->format('d/m/Y H:i') }}
                                                    @if(isset($item['status']) && $item['status'] !== 'submitted' && $item['status'] !== 'completed' && $item['status'] !== 'skipped')
                                                        <br>
                    ...
                    <!-- Approval Section -->
                    @if(in_array($permit->status, ['pending_class_teacher', 'pending_duty_teacher', 'pending_academic', 'pending_head']) && $canApprove)
                        <div class="info-card">
                            <div class="card-title">
                            ...
                            @if($permit->status === 'pending_class_teacher' && $teacher->role_id == 4)
                                @if(!$hasDutyTeacher)
                                    <div class="alert-info-custom">
                                        ...
                                        <strong>Taarifa:</strong> Hakuna mwalimu wa zamu kwenye duty roster kwa tarehe hii ({{ \Carbon\Carbon::parse($permit->departure_date)->format('d/m/Y') }}).
                                        Baada ya kuthibitisha kwako, ombi litaenda moja kwa moja kwa Mwalimu wa Taaluma.
                                    </div>
                                @else
                                    <div class="alert-info-custom">
                                        <i class="fas fa-check-circle me-2 text-success"></i>
                                        <strong>Taarifa:</strong> Kuna mwalimu wa zamu kwa tarehe hii. Baada ya kuthibitisha kwako, ombi litaenda kwa Mwalimu wa Zamu.
                                    </div>
                                @endif
                            @endif
                            ...
                            @if($permit->status === 'pending_duty_teacher' && $teacher->role_id == 3)
                                <div class="alert-info-custom">
                                    <i class="fas fa-info-circle me-2"></i>
                                    <strong>Taarifa:</strong> Unathibitisha ombi kwa niaba ya Mwalimu wa Zamu. Ombi litaenda moja kwa moja kwa Mwalimu Mkuu baada ya uthibitisho wako.
                                </div>
                            @endif
                            ...
                            @if($permit->status === 'pending_academic' && $teacher->role_id == 3)
                                <div class="alert-info-custom">
                                    <i class="fas fa-info-circle me-2"></i>
                                    <strong>Taarifa:</strong> Baada ya kuthibitisha kwako, ombi litaenda kwa Mwalimu Mkuu kwa uthibitisho wa mwisho.
                                </div>
                            @endif
                            ...
                            @if($permit->status === 'pending_head' && $teacher->role_id == 2)
                                <div class="alert-info-custom">
                                    <i class="fas fa-info-circle me-2"></i>
                                    <strong>Taarifa:</strong> Wewe ndiye ngazi ya mwisho ya uthibitisho. Baada ya kuthibitisha, ruhusa itatolewa na mzazi atapata kibali.
                                </div>
                            @endif
                    ...
                    @elseif(in_array($permit->status, ['pending_class_teacher', 'pending_duty_teacher', 'pending_academic', 'pending_head']) && !$canApprove)
                        <div class="info-card">
                            <div class="alert-info-custom">
                                <i class="fas fa-lock me-2"></i>
                                <strong>Kumbuka:</strong> Huna uwezo wa kuthibitisha ombi hili kwa sababu wewe si mwalimu aliyepewa jukumu la hatua hii.
                            </div>
                            <a href="{{ route('teacher.e-permit.dashboard') }}" class="btn-back">
                                <i class="fas fa-arrow-left"></i> Rudi Nyuma
                            </a>
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                ...
                .then(data => {
                    if (data.success) {
                        Swal.fire('Imefanikiwa!', data.message, 'success')
                            .then(() => window.location.href = '{{ route("teacher.e-permit.dashboard") }}');
                    } else {
                        Swal.fire('Hitilafu!', data.message, 'error');
                    }
                })

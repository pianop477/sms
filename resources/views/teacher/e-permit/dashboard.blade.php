                            ...
                            <div class="row align-items-center">
                                ...
                                <div class="col-md-9">
                                    <div class="alert alert-info mb-0 py-2">
                                        ...
                                        <small>Showing permit requests for year: <strong
                                                id="selectedYearDisplay">{{ $selectedYear }}</strong></small>
                                    </div>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="tab-content">
                            ...
                            <div class="tab-pane fade show active" id="pending" role="tabpanel">
                                <div class="filter-section">
                                    <div class="row align-items-end g-2">
                                        ...
                                        <div class="col-md-3">
                                            ...
                                            <input type="date" id="dateToPendingInput"
                                                class="form-control form-control-sm">
                                        </div>
                                        <div class="col-md-2">
                                ...
                                <div id="pendingTableContainer">
                                    @include('teacher.e-permit.partials.pending_table', [
                            ...
                            <div class="tab-pane fade" id="history" role="tabpanel">
                                <div class="filter-section">
                                    <div class="row align-items-end g-2">
                                        ...
                                        <div class="col-md-3">
                                            <label class="form-label fw-bold small">Date To</label>
                                            <input type="date" id="dateToHistoryInput"
                                                class="form-control form-control-sm">
                                        </div>
                                        <div class="col-md-2">
                                ...
                                <div id="historyTableContainer">
                                    @include('teacher.e-permit.partials.history_table', [
                            ...
                            <div class="tab-pane fade" id="reports" role="tabpanel">
                                ...
                                <div class="mb-3">
                                    <button id="exportPdfBtn" class="btn btn-danger btn-sm"><i
                                        ...
                                        <tbody>
                                            <tr>
                                                <td colspan="8" class="text-center py-4 text-muted">
                                                    ...
                                                    Select date range or status and click Filter to load reports
                                                </td>
                                            </tr>
                                        </tbody>
                                    </table>
                            ...
                            <div class="tab-pane fade" id="studentReturn" role="tabpanel">
                                ...
                                <div class="table-responsive" id="returnTableContainer">
                                    <table class="table permit-table">
                                        ...
                                        <tbody>
                                            <tr>
                                                <td colspan="7" class="text-center py-4 text-muted">Search for a permit
                                                    to
    ...
    <script>
        ...
        function loadPendingTable(page = 1) {
            ...
            fetch(url, {
                ...
                .catch(error => {
                    ...
                    if (container) {
                        container.innerHTML =
                            '<div class="alert alert-danger text-center">Error loading data. Please refresh the page and try again.</div>';
                    }
                });
        }
        ...
        function loadPendingTableWithUrl(url) {
            ...
            fetch(fetchUrl, {
                ...
                .catch(error => {
                    ...
                    if (container) {
                        container.innerHTML =
                            '<div class="alert alert-danger text-center">Error loading data. Please refresh the page.</div>';
                    }
                });
        }
        ...
        function loadHistoryTableWithUrl(url) {
            fetch(url, {
                ...
                .then(response => response.json())
                .then(data => {
                    if (data.success) document.getElementById('historyTableContainer').innerHTML = data.html;
                    attachHistoryPaginationHandlers();
                })
        ...
        function loadReports(page = 1) {
            ...
            if (!dateFrom && !dateTo && status === 'all') {
                if (tbody) {
                    tbody.innerHTML =
                        '<tr><td colspan="8" class="text-center py-4 text-muted">Please select date range or status to load reports</td></tr>';
                }
            ...
            fetch(`/teacher/e-permit/reports/data?date_from=${dateFrom}&date_to=${dateTo}&status=${status}&page=${page}`, {
                    headers: {
                        'X-Requested-With': 'XMLHttpRequest',
                        'X-CSRF-TOKEN': '{{ csrf_token() }}',
                        'Accept': 'application/json'
                ...
                .then(data => {
                    ...
                    if (data.success) {
                        ...
                        <tr>
                            <td><strong>${escapeHtml(permit.permit_number.toUpperCase())}</strong></td>
                            <td>${escapeHtml(permit.student_name)}<br><small class="text-muted">${escapeHtml(permit.admission_number.toUpperCase())}</small></td>
                            <td>${escapeHtml(permit.class_name)}</td>
                            <td>${escapeHtml(permit.guardian_name)}<br><small>${escapeHtml(permit.guardian_phone)}</small></td>
                            <td>${escapeHtml(permit.departure_date)}</td>
                            <td>${escapeHtml(permit.expected_return_date)}</td>
                            ...
                            <td>${escapeHtml(permit.created_date)}</td>
                        </tr>
                    `).join('');
                            }
                            ...
                            if (paginationContainer && data.last_page > 1) {
                                paginationContainer.innerHTML = generatePaginationHtml(data.current_page, data
                                    .last_page);
                                attachReportsPaginationHandlers();
                        ...
                        if (tbody) {
                            tbody.innerHTML =
                                '<tr><td colspan="8" class="text-center py-4 text-danger">Error loading data. Please try again.</td></tr>';
                        }
                    }
                })
                .catch(error => {
        ...
        function generatePaginationHtml(currentPage, lastPage) {
            let html = '<nav aria-label="Reports pagination"><ul class="pagination justify-content-end">';
        ...
        function attachReportsPaginationHandlers() {
            const paginationLinks = document.querySelectorAll('#reportsPagination .page-link[data-page]');
            paginationLinks.forEach(link => {
                link.addEventListener('click', function(e) {
                    e.preventDefault();
                    const page = this.getAttribute('data-page');
                    if (page) {
                        loadReports(parseInt(page));
                    }
                });
            });
        }
        ...
        document.getElementById('resetReportsBtn')?.addEventListener('click', function() {
            ...
            if (tbody) {
                tbody.innerHTML =
                    '<tr><td colspan="8" class="text-center py-4 text-muted">Select filters and click Filter to load reports</td></tr>';
            }
        });
        ...
        document.getElementById('exportPdfBtn')?.addEventListener('click', function() {
            const d = new Date();
            window.open(
                `/teacher/e-permit/reports/export-pdf?date_from=${document.getElementById('reportDateFrom')?.value || ''}&date_to=${document.getElementById('reportDateTo')?.value || ''}&status=${document.getElementById('reportStatus')?.value || 'all'}&_t=${d.getTime()}`,
                '_blank');
        });
        document.getElementById('exportExcelBtn')?.addEventListener('click', function() {
            const d = new Date();
            window.open(
                `/teacher/e-permit/reports/export-excel?date_from=${document.getElementById('reportDateFrom')?.value || ''}&date_to=${document.getElementById('reportDateTo')?.value || ''}&status=${document.getElementById('reportStatus')?.value || 'all'}&_t=${d.getTime()}`,
                '_blank');
        });
        document.querySelector('#ePermitTab button[data-bs-target="#reports"]')?.addEventListener('shown.bs.tab',
            loadReports);
        ...
        document.getElementById('searchReturnBtn')?.addEventListener('click', function() {
            ...
            fetch(`/teacher/e-permit/return/search?search=${encodeURIComponent(search)}`, {
                ...
                .catch(error => {
                    ...
                    Swal.fire('Hitilafu', 'Tafadhali jaribu tena. Hakikisha umeingiza taarifa sahihi.',
                        'error');
                });
        });
        ...
        function populateReturnRelationships() {
            ...
            if (selectedType === 'parent') {
                ['Baba', 'Mama'].forEach(rel => options += `<option value="${rel.toLowerCase()}">${rel}</option>`);
            } else if (selectedType === 'guardian') {
                ['Dada', 'Kaka', 'Shangazi', 'Mjomba', 'Babu', 'Bibi'].forEach(rel => options +=
                    `<option value="${rel.toLowerCase()}">${rel}</option>`);
            }
        ...
        function displayReturnInfo(permit) {
            ...
            const studentImage = permit.student.image ? `/storage/students/${permit.student.image}` :
        ...
        `;
            ...
            document.getElementById('return_alone_yes')?.addEventListener('change', () => document.getElementById(
                'returnAccompaniedFields').style.display = 'none');
            document.getElementById('return_alone_no')?.addEventListener('change', () => document.getElementById(
                'returnAccompaniedFields').style.display = 'block');
                ...
                Swal.fire({
                    ...
                    if (result.isConfirmed) {
                        ...
                        fetch(`/teacher/e-permit/return/${permit.id}/confirm`, {
                            ...
                            .then(response => response.json())
                            .then(data => {
                                if (data.success) Swal.fire('Imefanikiwa!', data.message, 'success')
